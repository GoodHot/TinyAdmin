  ...
  <div style="position: relative">
    ...
    <vue-nestable v-model="nestableItems" @input="inputHandler" @change="changeHandler" ref="nestable">
      <vue-nestable-handle slot-scope="{ item }" :item="item">
        <b-button class="mr-1" type="is-text" size="is-small" style="text-decoration: none;" v-if="item.children && item.children.length > 0" @click="expandHandler(item)">{{ item.expand }}</b-button>
